...
namespace CPU_serial_gng
{
    ...
    vector<double> getRandomVector(const unsigned int featureSpaceDimension)
    {
        ...
        for (unsigned int i = 0; i != featureSpaceDimension; ++i)
            randomVec.push_back(getRandomFloat(0.0, 1.0));
        return randomVec;
    }
    ...
    //Initilaizes network from a file
    ...
    void NeuralGasNetwork::AddNode()
    {
        ...
        largestErrorNeighbor = largestErrorNode->edges[0]->neighboringNode(largestErrorNode);
        ...
        for (int i = 0; i != largestErrorNode->edges.size(); ++i)
        {
            ...
            if (otherNode->error > maxNeighborError)
            {
                ...
                //cout << (largestErrorNeighbor == largestErrorNode) << endl;
                //cout << (largestErrorNode->edges[i]->node1 == largestErrorNode->edges[i]->node2) << endl;
            }
        }
    ...
    void NeuralGasNetwork::iterate(const vector<double> &featureVector)
    {
        ...
        for (auto it = closestNode->edges.begin(); it != closestNode->edges.end(); ++it)
        {
            if ((*it)->neighboringNode(closestNode) == secondClosestNode)
            {
                (*it)->age = 0;
    ...
    void NeuralGasNetwork::writeToFile()
    {
        ...
        */
        ...
        fileNameStream << "GNG-NN.net";
        netFile.open(fileNameStream.str());

        //Write the number of nodes
        netFile << nodes.size() << endl;

        //First go through the nodes and assign to each one the updated node index while also
        //writing the node's reference vector
        int k = 0;
        for (unsigned int i = 0; i != nodes.size(); ++i)
        {
            nodes[i]->nodeIndex = i;
            for (unsigned int j = 0; j != nodes[i]->referenceVector.size(); ++j)
                netFile << nodes[i]->referenceVector[j] << " ";
            netFile << endl;
            k += nodes[i]->edges.size();
        }

        //Write all the edges
        for (unsigned int i = 0; i != edges.size(); ++i)
        {
            netFile << edges[i]->node1->nodeIndex << " ";
            netFile << edges[i]->node2->nodeIndex << " ";
            netFile << edges[i]->age << endl;
        }
        netFile.close();
    }
    ...
    void NeuralGasNetwork::readFromFile(const string fileName)
    {
        ...
        - ...  
        */
        std::ifstream netFile(fileName, std::ios::in | std::ios::binary);
        string line;
        ...
        while (std::getline(netFile, line))
        {
            ...
            else if (lineNumber-1 < numberOfNodes) //if parsing the reference vectors
            {
                vector<double> refVec;
                double val;
                while (lineStream >> val)
                    refVec.push_back(val); //load the reference vectors
                NeuralGasNode* newNode = new NeuralGasNode(refVec); //create the new node
                nodes.push_back(newNode); //push it onto this.nodes vector
            }
            else //parsing edges
            {
                ...
                unsigned int age;
                lineStream >> node1Index >> node2Index >> age; //parse the line

                //create the edge
                NodeEdge* newEdge = new NodeEdge(nodes[node1Index], nodes[node2Index], age);

                //Add the edge to the appropriate edge vectors
                nodes[node1Index]->edges.push_back(newEdge);
                nodes[node2Index]->edges.push_back(newEdge);
                edges.push_back(newEdge);
                k += 1;
            }
            ++lineNumber;
    ...
    void NeuralGasNetwork::setMaxNodes(unsigned int newMaxNodes)
    {
